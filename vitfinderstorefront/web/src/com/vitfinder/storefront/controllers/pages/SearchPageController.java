...
public class SearchPageController extends AbstractSearchPageController
{
			...
			final HttpServletRequest request, final Model model) throws CMSItemNotFoundException
	{
		if (StringUtils.isNotBlank(searchText))
		{
			...
			searchPageData.setFreeTextSearch(
					xssFilter(org.apache.commons.lang.StringEscapeUtils.unescapeHtml(searchPageData.getFreeTextSearch())));
			parseSuggetions(searchPageData);
		}
		else
		{
			storeCmsPageInModel(model, getContentPageForLabelOrId(NO_RESULTS_CMS_PAGE_ID));
	 ...
	 */
	private void parseSuggetions(final ProductSearchPageData<SearchStateData, ProductData> searchPageData)
	{
		// YTODO Auto-generated method stub
		if (searchPageData.getSpellingSuggestion() != null)
		{
			final SpellingSuggestionData<SearchStateData> suggetions = searchPageData.getSpellingSuggestion();
			suggetions.setSuggestion(xssFilter(org.apache.commons.lang.StringEscapeUtils.unescapeHtml(suggetions.getSuggestion())));
		}
	 ...
	 * @param model
	 */
	private void parseBreadCrumbs(final Model model)
	{
		final List<Breadcrumb> breadCrumbs = (List<Breadcrumb>) model.asMap().get(WebConstants.BREADCRUMBS_KEY);
		if (breadCrumbs != null)
		{
			for (final Breadcrumb breadCrumb : breadCrumbs)
			{
				Breadcrumb searchBreadcrumb = null;
				String name = org.apache.commons.lang.StringEscapeUtils.unescapeHtml(breadCrumb.getName());
				name = xssFilter(name);
				searchBreadcrumb = new Breadcrumb(breadCrumb.getUrl(), name, breadCrumb.getLinkClass(), breadCrumb.getCategoryCode());
	...
	private String xssFilter(final String value)
	{
		if (value == null)
		{
			return null;
		...
		sanitized = sanitized.replaceAll("&40;", "\\(").replaceAll("&41;", "\\)");
		sanitized = sanitized.replaceAll("&amp;39&#x3b;", "'");
		sanitized = sanitized.replaceAll("&amp;40&#x3b;", "\\(");
		sanitized = sanitized.replaceAll("&amp;41&#x3b;", "\\)");
		sanitized = sanitized.replaceAll("&39;", "'");
		sanitized = sanitized.replaceAll("&39;", "'");
		return sanitized;
	}
	...
	protected void updatePageTitle(final String searchText, final Model model)
	{
		storeContentPageTitleInModel(model,
				getPageTitleResolver().resolveContentPageTitle(getMessageSource().getMessage("search.meta.title", null,
						...
						+ xssFilter(org.apache.commons.lang.StringEscapeUtils.unescapeHtml(searchText))));
	}
