...
import com.vitfinder.facades.checkout.VitfinderCheckoutFacade;
...
public class CartPageController extends AbstractCartPageController
{
	...
	@Resource(name = "vitfinderCheckoutFacade")
	private VitfinderCheckoutFacade checkoutFacade;
	...
	public String cartCheck(final Model model, final RedirectAttributes redirectModel,
			@RequestParam(value = "retailerCartId", required = false) final String retailerCartId,
			@RequestParam(value = "sortBean", required = false) final String sortBean)
			throws CommerceCartModificationException, CMSItemNotFoundException
	{
		...
		{
			if (getCartFacade() instanceof VitfinderCartFacade)
			{
				((VitfinderCartFacade) getCartFacade()).updateGenericCartEntry(sortBean);
			}
		}
		showCart(model);
		model.addAttribute("retailerCartId", retailerCartId);
		if (sortBean != null && !sortBean.isEmpty())
		{
			if (getCartFacade() instanceof VitfinderCartFacade)
			{
				CartData cartData = (CartData) model.asMap().get("cartData");
				cartData = ((VitfinderCartFacade) getCartFacade()).doRetailerBasketSort(cartData, sortBean);
				model.addAttribute("sortOption", sortBean);
			}
		}
		return ControllerConstants.Views.Pages.Cart.ComparePage;
	}
	...
	public String updateRetailerCartQuantities(@RequestParam("entryNumber") final long entryNumber, final Model model,
			@Valid final UpdateQuantityForm form, final BindingResult bindingResult, final HttpServletRequest request,
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		final String retailerCartId = request.getParameter("retailerCartId");
		...
		else if (getCartFacade().hasEntries())
		{
			try
			{
				if (getCartFacade() instanceof VitfinderCartFacade)
				{
					final CartModificationData cartModification = ((VitfinderCartFacade) getCartFacade())
							.updateRetailerCartEntry(entryNumber, form.getQuantity().longValue(), retailerCartId);
					...
					model.addAttribute("retailerCartId", retailerCartId);
					request.setAttribute("retailerCartId", retailerCartId);
					cartCheck(model, redirectModel, retailerCartId, null);
				}
			}
	...
	public String getDeliverCosts(@RequestParam("retailerName") final String retailerName, final Model model,
			final RedirectAttributes redirectModel) throws CommerceCartModificationException, CMSItemNotFoundException
	{
		prepareDataForPage(model);
		model.addAttribute(WebConstants.BREADCRUMBS_KEY, resourceBreadcrumbBuilder.getBreadcrumbs("breadcrumb.cart"));
		model.addAttribute("pageType", PageType.CART.name());
		model.addAttribute("deliveryMethods", checkoutFacade.getSupportedDeliveryModes(retailerName));
		return ControllerConstants.Views.Fragments.Cart.DeliveryCostPopup;
	}


	@RequestMapping(value = "/basketSummary/{retailerBasketCode}", method = RequestMethod.GET)
	public String showRetailerBasket(@PathVariable("retailerBasketCode") final String retailerBasketCode, final Model model)
			throws CMSItemNotFoundException
	{
		prepareRetailerBasket(model, retailerBasketCode);
		return ControllerConstants.Views.Pages.Cart.SummaryPage;
	}


	@RequestMapping(value = "/retailerCheckout/{retailerBasketCode}", method = RequestMethod.GET)
	public String retailerCheckout(@PathVariable("retailerBasketCode") final String retailerBasketCode, final Model model)
			throws CMSItemNotFoundException
	{

		prepareRetailerBasket(model, retailerBasketCode);
		return ControllerConstants.Views.Pages.Cart.RetailerPage;
	}

	private void prepareRetailerBasket(final Model model, final String retailerBasketCode) throws CMSItemNotFoundException
	{
		prepareDataForPage(model);
		final CartData cartData = cartFacade.getSessionCartWithEntryOrdering(true);

		final List<CartData> retailerBaskets = cartData.getRetailerBaskets();
		for (final CartData retailerCart : retailerBaskets)
		{
			if (retailerCart.getCode().equals(retailerBasketCode))
			{
				model.addAttribute("cartData", retailerCart);
			}
		}
	...
	//vtf-213
	@RequestMapping(value = "/basketSummary/update", method = RequestMethod.POST)
	public String updateRetailerSummaryCartQuantities(@RequestParam("entryNumber") final long entryNumber, final Model model,
			@Valid final UpdateQuantityForm form, final BindingResult bindingResult, final HttpServletRequest request,
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		final String retailerCartId = request.getParameter("retailerCartId");
		...
		else if (getCartFacade().hasEntries())
		{
			try
			{
				if (getCartFacade() instanceof VitfinderCartFacade)
				{
					...
					//prepareRetailerBasket(model, retailerCartId);
					//return ControllerConstants.Views.Pages.Cart.SummaryPage;
					return REDIRECT_PREFIX + "/cart/basketSummary/" + retailerCartId;
				}
			}
			catch (final CommerceCartModificationException ex)
			{
