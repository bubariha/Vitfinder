...
import com.vitfinder.facades.customer.form.UpdateCommPrefForm;
import com.vitfinder.facades.customer.impl.DefaultVitfinderCustomerFacade;
...
public class AccountPageController extends AbstractSearchPageController
{
	...
	private static final String REDIRECT_TO_UPDATE_COMM_PREF_PAGE = REDIRECT_PREFIX + "/my-account/update-comm-pref";
	...
	private static final String UPDATE_COMM_PREF_CMS_PAGE = "update-comm-pref";
	private static final String STUDY_MY_BAND = "study-my-band";
	...
	@Resource(name = "vitfinderPasswordValidator")
	...
	@RequestMapping(value = "/update-comm-pref", method = RequestMethod.GET)
	@RequireHardLogIn
	public String editCommPref(final Model model) throws CMSItemNotFoundException
	{
		final CustomerData customerData = customerFacade.getCurrentCustomer();
		final UpdateCommPrefForm updateCommPrefForm = new UpdateCommPrefForm();

		updateCommPrefForm.setCommunicationPreferences(customerData.getCommunicationPreferences() != null
				? customerData.getCommunicationPreferences().getListOfPreference() : new ArrayList<String>());
		updateCommPrefForm.setRemainder(String.valueOf(
				customerData.getCommunicationPreferences() != null ? customerData.getCommunicationPreferences().getRemainder() == null
						? "" : customerData.getCommunicationPreferences().getRemainder() : ""));

		model.addAttribute("updateCommPrefForm", updateCommPrefForm);
		model.addAttribute("listOfRemainders", prepareRemainders());
		model.addAttribute("communicationPreferences", prepareCommPrefs());
		storeCmsPageInModel(model, getContentPageForLabelOrId(UPDATE_COMM_PREF_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(UPDATE_COMM_PREF_CMS_PAGE));
		model.addAttribute("breadcrumbs", accountBreadcrumbBuilder.getBreadcrumbs("text.account.profile"));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		return getViewForPage(model);
	}
	 ...
	 * @return
	 */
	private List<String> prepareCommPrefs()
	{
	...
	private List<Integer> prepareRemainders()
	{
		final List<Integer> remainders = new ArrayList<Integer>();
		remainders.add(30);
		remainders.add(60);
		remainders.add(90);
		return remainders;
	...
	@RequestMapping(value = "/update-comm-pref", method = RequestMethod.POST)
	@RequireHardLogIn
	public String editCommPref(final UpdateCommPrefForm UpdateCommPrefForm, final BindingResult bindingResult, final Model model,
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{

		final String returnAction = REDIRECT_TO_UPDATE_COMM_PREF_PAGE;
		if (customerFacade instanceof DefaultVitfinderCustomerFacade)
		{
			((DefaultVitfinderCustomerFacade) customerFacade)
					.updateCommunicationPreferences(prepareCustomerData(UpdateCommPrefForm));
		}
		GlobalMessages.addFlashMessage(redirectAttributes, GlobalMessages.CONF_MESSAGES_HOLDER,
				"text.account.profile.confirmationUpdated", null);
	...
	@RequestMapping(value = "/study-my-band", method = RequestMethod.GET)
	@RequireHardLogIn
	public String studyMyBand(final Model model) throws CMSItemNotFoundException
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(STUDY_MY_BAND));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(STUDY_MY_BAND));
		model.addAttribute("breadcrumbs", accountBreadcrumbBuilder.getBreadcrumbs("text.account.profile.studymyband"));
		return ControllerConstants.Views.Pages.Account.StudyMyBand;
	}
	 ...
	 * @return
	 */
	private CustomerData prepareCustomerData(final UpdateCommPrefForm updateCommPrefForm)
	{
		// YTODO Auto-generated method stub
		final CustomerData customerData = new CustomerData();
		final CommunicationPreferenceData commPrefData = new CommunicationPreferenceData();
		customerData.setCommunicationPreferences(commPrefData);
		final List<String> data = updateCommPrefForm.getCommunicationPreferences() == null ? new ArrayList<String>()
				: updateCommPrefForm.getCommunicationPreferences();
		commPrefData.setListOfPreference(data);
		commPrefData
				.setRemainder(updateCommPrefForm.getRemainder() == null ? null : Integer.valueOf(updateCommPrefForm.getRemainder()));
		return customerData;
	}
