...
public class VitfinderCommerceUpdateCartEntryStrategy extends DefaultCommerceUpdateCartEntryStrategy
{
			...
			throws CommerceCartModificationException
	{
		...
		final VitfinderGenericProductModel genericProductModel = entryToUpdate.getGenericProduct();
		...
		if (genericProductModel == null)
		{
			...
			if (retailerBaskets != null && retailerBaskets.size() > 0)
			{
				for (final CartModel retailerBasket : retailerBaskets)
				{
					final String eanNumber = entryToUpdate.getProduct().getEan();
					if (eanNumber != null)
					{
						//
						final long retailerEntryNumber = getProductEntryNumberOfretailerBasket(entryToUpdate, retailerBasket);
						if (retailerEntryNumber >= 0)
						{
							//
							parameters.setCart(retailerBasket);
							parameters.setEntryNumber(retailerEntryNumber);
							updateRetailerCartEntry(parameters);
						}
					}
				}
			}
		...
		{
			//it is generic product operation compare with generic product
			final List<CartModel> retailerBaskets = cartModel.getRetailerBaskets();
			if (retailerBaskets != null && retailerBaskets.size() > 0)
			{
				for (final CartModel retailerBasket : retailerBaskets)
				{
					final VitfinderGenericProductModel genericProduct = entryToUpdate.getGenericProduct();
					if (genericProduct != null)
					{
						final long retailerEntryNumber = getGenericProductEntryNumberOfretailerBasket(entryToUpdate, retailerBasket);
						if (retailerEntryNumber >= 0)
						{
							parameters.setCart(retailerBasket);
							parameters.setEntryNumber(retailerEntryNumber);
	 ...
	 * @return
	 */
	private long getGenericProductEntryNumberOfretailerBasket(final AbstractOrderEntryModel entryToUpdate,
			final CartModel retailerBasket)
	{
		// YTODO Auto-generated method stub
		final VitfinderGenericProductModel retailergenericProduct = entryToUpdate.getGenericProduct();
		long entryNumber = 0;
		for (final AbstractOrderEntryModel retailerCartEntry : retailerBasket.getEntries())
		{
			final ProductModel entryProduct = retailerCartEntry.getGenericProduct();
			if (entryProduct != null && entryProduct.getCode().equalsIgnoreCase(retailergenericProduct.getCode()))
			{
				return entryNumber;
			}
			entryNumber++;
	 ...
	 * @throws CommerceCartModificationException
	 */
	private CommerceCartModification updateRetailerCartEntry(final CommerceCartParameter parameters)
			throws CommerceCartModificationException
	{
		beforeUpdateCartEntry(parameters);
		final CartModel cartModel = parameters.getCart();
	 ...
	 * @return
	 */
	protected long getProductEntryNumberOfretailerBasket(final AbstractOrderEntryModel entryToUpdate,
			final CartModel retailerBasket)
	{
		// YTODO Auto-generated method stub
		final String retailerEan = entryToUpdate.getProduct().getEan();
		long entryNumber = 0;
		for (final AbstractOrderEntryModel retailerCartEntry : retailerBasket.getEntries())
		{
			final ProductModel entryProduct = retailerCartEntry.getProduct();
			if (entryProduct != null && retailerEan.equalsIgnoreCase(entryProduct.getEan()))
			{
				return entryNumber;
			}
			entryNumber++;
	 ...
	 * @throws CommerceCartModificationException
	 */
	public CommerceCartModification updateQuantityForGenericCartEntry(final CommerceCartParameter parameters)
			throws CommerceCartModificationException
	{
		...
		if (entryToUpdate.getGenericProduct() == null)
		{
			final List<CartModel> retailerBaskets = cartModel.getRetailerBaskets();
			if (retailerBaskets != null && retailerBaskets.size() > 0)
			{
